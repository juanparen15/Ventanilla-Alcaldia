...
class RadicadoController extends VoyagerBaseController
{
    ...
    public function index(Request $request)
    {
        ...
        // Obtener datos de la tabla de radicados
        $radicados = ArmorumappRadicado::currentUser()->get();

        // Filtrar solicitudes del usuario autenticado con role_id = 2
        $solicitudes = ArmorumappSolicitud::currentUser()->get();

        // Obtener los DataTypes de Radicados y Solicitudes
        $dataTypeRadicados = Voyager::model('DataType')->where('slug', '=', 'armorumapp-radicado')->first();
        $dataTypeSolicitudes = Voyager::model('DataType')->where('slug', '=', 'armorumapp-solicitud')->first();

        // Obtener datos de Radicados
        $radicados = app($dataTypeRadicados->model_name)->currentUser()->get();

        // Obtener datos de Solicitudes filtradas
        $solicitudes = app($dataTypeSolicitudes->model_name)->currentUser()->get(); // Filtrar por usuario autenticado
