...
@section('javascript')
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            document.querySelectorAll('.detalle-inscripcion').forEach(function(button) {
                button.addEventListener('click', function(e) {
                    e.preventDefault(); // Previene cualquier comportamiento de redirección

                    const inscripcionId = this.getAttribute('data-id');
                    const url = `/inscripciones/${inscripcionId}/detalle`;
                    ...
                    fetch(url)
                        ...
                        .then(data => {
                            if (data.error) {
                                ...
                                // Construye un HTML para mostrar los detalles
                                let detallesHtml =
                                    '<table class="table table-bordered" style="width: 100%; overflow-x: auto;"><thead><tr><th>Código Inscripción</th><th>Documento</th><th>Usuario</th><th>Evento</th><th>Armas</th><th>Puntaje</th><th>Observaciones</th></tr></thead><tbody>';
                                data.detalles.forEach(detalle => {
                                    detallesHtml += `<tr>
                                    <td>${detalle.codigo_inscripcion || 'No disponible'}</td>
                                    <td>${detalle.documento_tercero || 'No disponible'}</td>
                                    <td>${detalle.usuario || 'No disponible'}</td>
                                    <td>${detalle.eventoDetalle || 'No disponible'}</td>
                                    <td>${detalle.armas.length > 0 ? detalle.armas.join('<br>') : 'No disponible'}</td>
                                    <td>${detalle.puntaje || 'No disponible'}</td>
                                ...
                                Swal.fire({
                                    ...
                                    html: detallesHtml,
                                    icon: 'info',
                                    width: '90%', // Asegura que el modal sea amplio en dispositivos móviles
                                    customClass: {
                        ...
                        .catch(error => {
                            Swal.fire('Error',
                                'No se pudieron cargar los detalles de la inscripción.',
                                'error');
                        });
                });
            });
...
<style>
    ...
    .responsive-swal table {
        width: 100%;
        border-collapse: collapse; /* Elimina los espacios entre celdas */
    }
    
    .responsive-swal th, .responsive-swal td {
    ...
    /* Estilos para tablets (ancho de pantalla de 768px a 1024px) */
    @media (min-width: 768px) and (max-width: 1024px) {
        .responsive-swal th, .responsive-swal td {
            padding: 10px; /* Menor espacio en pantallas más pequeñas */
        }
    }

    /* Estilos para computadoras (pantallas más grandes) */
    @media (min-width: 1024px) {
        .responsive-swal th, .responsive-swal td {
            padding: 12px; /* Espacio estándar en pantallas grandes */
        }
    }
</style>
