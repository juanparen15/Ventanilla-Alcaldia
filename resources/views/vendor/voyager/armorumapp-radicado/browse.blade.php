...
@section('page_title', __('voyager::generic.viewing') . ' Radicados y Solicitudes')
...
@section('page_header')
    <div class="container-fluid">
        <h1 class="page-title">
            Radicados
            &nbsp;|&nbsp;
            <i class="{{ $dataTypeSolicitudes->icon }}"></i> Solicitudes
        </h1>
        @can('add', app($dataTypeRadicados->model_name))
            <a href="{{ route('voyager.' . $dataTypeRadicados->slug . '.create') }}" class="btn btn-success btn-add-new">
                <i class="voyager-plus"></i> <span>{{ __('voyager::generic.add_new') }}</span> Radicado
            </a>
        @endcan
        @can('add', app($dataTypeSolicitudes->model_name))
            <a href="{{ route('voyager.' . $dataTypeSolicitudes->slug . '.create') }}" class="btn btn-success btn-add-new">
                <i class="voyager-plus"></i> <span>{{ __('voyager::generic.add_new') }}</span> Solicitud
            </a>
        @endcan
    </div>
@stop
            ...
            <div class="panel-body">
                ...
                @if ($isServerSide)
                    <form method="get" class="form-search">
                        <div id="search-input">
                            <div class="col-2">
                                <select id="search_key_radicados" name="key">
                            ...
                            <div class="col-2">
                                <select id="filter_radicados" name="filter">
                ...
                <div class="table-responsive">
                    <table id="dataTableRadicados" class="table table-hover">
                        <thead>
                            <tr>
                                ...
                                @endif
                                @foreach($dataTypeSolicitudes->browseRows as $row)
                                <th>
                                ...
                                @foreach($dataTypeSolicitudes->browseRows as $row)
                                    ...
                                    <td>
                                        @if (isset($row->details->view_browse))
                                            @include($row->details->view_browse, ['row' => $row, 'dataType' => $dataTypeSolicitudes, 'dataTypeContent' => $solicitudes, 'content' => $data->{$row->field}, 'view' => 'browse', 'options' => $row->details])
                                        @elseif (isset($row->details->view))
                                            @include($row->details->view, ['row' => $row, 'dataType' => $dataTypeSolicitudes, 'dataTypeContent' => $solicitudes, 'content' => $data->{$row->field}, 'action' => 'browse', 'view' => 'browse', 'options' => $row->details])
                ...
                @if ($isServerSide)
                    <div class="pull-left">
                        <div role="status" class="show-res" aria-live="polite">
                            {{ trans_choice('voyager::generic.showing_entries', $solicitudes->total(), [
                                'from' => $solicitudes->firstItem(),
                                'to' => $solicitudes->lastItem(),
                                'all' => $solicitudes->total(),
                            ]) }}
                        </div>
                    </div>
            ...
            <div class="panel-body">
                ...
                @if ($isServerSide)
                    <form method="get" class="form-search">
                        <div id="search-input">
                            <div class="col-2">
                                <select id="search_key_solicitudes" name="key">
                                    @foreach ($searchNames as $key => $name)
                                        <option value="{{ $key }}"
                                            @if ($search->key == $key || (empty($search->key) && $key == $defaultSearchKey)) selected @endif>{{ $name }}
                                        </option>
                            ...
                            <div class="col-2">
                                <select id="filter_solicitudes" name="filter">
                                    <option value="contains" @if ($search->filter == 'contains') selected @endif>
                                        {{ __('voyager::generic.contains') }}</option>
                                    <option value="equals" @if ($search->filter == 'equals') selected @endif>=
                                    </option>
                ...
                <div class="table-responsive">
                    <table id="dataTableSolicitudes" class="table table-hover">
                        <thead>
                            <tr>
                                @if ($showCheckboxColumn)
                                    <th class="dt-not-orderable">
    ...
    <script>
        $(document).ready(function() {
            @if (!$dataType->server_side)
                var tableRadicados = $('#dataTableRadicados').DataTable({!! json_encode(
                    array_merge(
                        [
                            'order' => $orderColumn,
                ...
                var tableSolicitudes = $('#dataTableSolicitudes').DataTable({!! json_encode(
                    array_merge(
                        [
                            'order' => $orderColumn,
                            'language' => __('voyager::datatable'),
            ...
            @if ($isModelTranslatable)
                ...
                $('#dataTableRadicados, #dataTableSolicitudes').on('draw.dt', function() {
        ...
        @if ($usesSoftDeletes)
            ...
            $(function() {
                $('#show_soft_deletes').change(function() {
                    ...
                    $('<a id="redir" href="' + redirUrl + '"></a>').appendTo('body').click();
                });
            });
        @endif
